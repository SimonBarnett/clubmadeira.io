...
// Handle category form submission (Generate or Refine Categories)
window.handleCategorySubmit = async function(event) {
    ...
    try {
        ...
        if (data.status === 'error' || data.error_message) {
            console.warn('handleCategorySubmit - API returned error:', data.error_message);
            toastr.error(`${data.error_message} <a href="#" onclick="document.getElementById('category-form').requestSubmit()">Retry</a>`);
            updateCategoriesSection({
                ...
                selected: formData.getlist ? formData.getlist('selected') : [],
        ...
        toastr.error(`Error: ${error.message} <a href="#" onclick="document.getElementById('category-form').requestSubmit()">Retry</a>`);
    } finally {
...
window.handleSaveCategories = async function() {
    ...
    try {
        ...
        if (data.status === 'error' || data.error_message) {
            throw new Error(data.error_message || 'Failed to save categories.');
        }

        toastr.success(data.message || 'Categories saved successfully.');
    } catch (error) {
...
window.handleResetCategories = async function() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Failed to reset categories: ${response.status}`);
        const data = await response.json();

        if (data.status === 'error' || data.error_message) {
            throw new Error(data.error_message || 'Failed to reset categories.');
        }

        toastr.success(data.message || 'Categories reset successfully.');
        updateCategoriesSection(data);
    } catch (error) {
...
async function loadCategories(userId, isAdmin) {
    ...
    try {
        ...
        if (data.status === 'error' || data.error_message) {
            throw new Error(data.error_message || 'Failed to load categories.');
        }

        data.categories = data.categories || {};
        data.deselected = data.deselected || [];
        data.previous_deselected = data.previous_deselected || [];
